...
public class HomePage {
	...
	public void validateItemsSearched() {
		for(WebElement searchList : searcheditems) {
			softAssert.assertTrue(searchList.getText().contains("MacBook") || searchList.getText().contains("Mac") || searchList.getText().contains("Apple"));
		}
	}
	...
	public void validatePriceRangeFilter() {
        ...
        for (WebElement priceElement : priceList) {
            try {
                ...
                boolean isSponsored = !parentDiv.findElements(By.xpath(".//*[contains(@class,'puis-sponsored-label-text')]")).isEmpty();
            ...
            try {
                ...
                softAssert.assertTrue(numPrice >= minPrice && numPrice <= maxPrice, "The Price is not in the correct range: " + numPrice);
    ...
    public void increaseMinPrice(WebElement increasePriceSlider, int xOffset) {
        ...
        ((JavascriptExecutor) driver).executeScript("arguments[0].value = arguments[1];", increasePriceSlider, newValue);
    ...
    public void decreaseMaxPrice(WebElement decreasePriceSlider, int xOffset) {
        ...
        ((JavascriptExecutor) driver).executeScript("arguments[0].value = arguments[1];", decreasePriceSlider, newValue);
        ((JavascriptExecutor) driver).executeScript("arguments[0].dispatchEvent(new Event('input'));", decreasePriceSlider);
    }
